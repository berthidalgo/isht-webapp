...
**Última actualización:** 5 de junio de 2026 — 15:20 (tras cierre de F0)
...
**Repo real:** `github.com/berthidalgo/isht-webapp` (rama `main`)
**Estado actual:** ✅ **Fase 0 COMPLETA y DESPLEGADA** — front+back vivos y conectados. Siguiente: **Fase 1 (ETL, arrancando por GeoPerú/admisibilidad)**.
**URLs en producción:**
- Backend (FastAPI/Render): **https://isht-webapp.onrender.com** — `/health` y `/docs` operativos
- Frontend (React/Vercel): **https://isht-webapp.vercel.app** — mapa de Perú + badge "backend OK"
**Documento estratégico hermano:** `ISHT_PLAN_MAESTRO_GEOTON2026.md`

> **⏩ PUNTO DE CONTINUACIÓN (para retomar en otro chat/agente).** Si estás leyendo esto en una sesión nueva: F0 ya está hecha y verificada (ver §14 — Bitácora de continuación al final). El trabajo inmediato es **F1**, y dentro de F1 lo PRIMERO y bloqueante es bajar la capa de cuencas Pfafstetter desde GeoPerú/GEOIDEP vía WFS (admisibilidad del concurso). El usuario eligió la vía: *"script que verifica endpoints WFS + descarga, corrido en su máquina"*. Aún NO se ha entregado ese script. Empezar por ahí. Detalle completo en §14.
...
```
F0 (Blueprint + esqueleto desplegado, front+back)  ── ✅ HECHO (15:20)
F1 (ETL: GeoPerú/GEOIDEP + PISCO + demanda → brecha) ─ ◀ AQUÍ ESTAMOS  núcleo de datos
F2 (ML: modelo de brecha entrenado y persistido)    ── pendiente  inteligencia
F3 (Backend API: índice + predicción + simulación)  ── pendiente (código base ya en el repo) servir el cerebro
F4 (Frontend interactivo: mapa + sliders + proyección) pendiente  el anexo visual
F5 (Pulido + deploy público + entregable Facilita)  ── pendiente  cierre y registro
```
...
### Lección #ISHT-3 — 2026-06-05 — Deploy F0: el Root Directory en monorepo
- **Problema:** Primer build en Render falló con `Could not open requirements file: requirements.txt`.
- **Causa raíz:** En un monorepo, Render/Vercel arrancan en la raíz del repo por defecto. El `requirements.txt` vive en `backend/`, no en la raíz.
- **Solución:** Fijar **Root Directory = `backend`** en Render (y **`frontend`** en Vercel). Con eso, cada plataforma entra a su subcarpeta antes de ejecutar build/start. El `render.yaml` ya trae `rootDir: backend`, pero solo aplica si se despliega como Blueprint; en Web Service manual hay que ponerlo a mano.
- **Impacto en ADN:** En cualquier deploy de monorepo, el primer setting que se verifica es el Root Directory de cada servicio.

### Lección #ISHT-4 — 2026-06-05 — Deploy F0: clavar la versión de Python
- **Problema:** Segundo build falló compilando `pydantic-core` (Rust) con `Read-only file system` y `maturin failed`.
- **Causa raíz:** Render usó Python 3.14 por defecto. Para 3.14 no hay ruedas (`.whl`) precompiladas de `pydantic-core`, `pandas`, `scikit-learn`, así que pip intentó compilarlas desde fuente, y el entorno de build tiene el cargo cache en solo-lectura.
- **Solución:** Clavar **Python 3.11.9** por dos vías redundantes: variable de entorno `PYTHON_VERSION=3.11.9` en Render **y** archivo `backend/.python-version` en el repo. En 3.11 todas bajan como binario `cp311...whl` (cero compilación). Tras el fix: `Build successful 🎉`.
- **Regla de selección de versión:** usar una versión de Python con ~1-2 años de madurez (no la más nueva) para garantizar ruedas precompiladas de la stack científica (numpy/pandas/scikit/xgboost). La punta de lanza del lenguaje es enemiga del deploy rápido.
- **Impacto en ADN:** En proyectos con stack científico + deadline, fijar versión de runtime conservadora es parte del esqueleto, no un afterthought.

### Lección #ISHT-5 — 2026-06-05 — Deploy F0: Vite hornea las env vars en build-time
- **Problema:** Frontend desplegado mostraba "backend sin respuesta" y `API_URL: http://localhost:8000` (el fallback), pese a haber configurado `VITE_API_URL` en Vercel.
- **Causa raíz:** Vite **inyecta las variables `VITE_*` en el momento del build**, no en runtime. La variable se añadió pero el sitio ya estaba construido con el valor viejo (o sin valor). Cambiar la variable sin redesplegar no surte efecto.
- **Solución:** Tras fijar `VITE_API_URL=https://isht-webapp.onrender.com` en el entorno **Production** de Vercel, forzar **Redeploy sin build-cache**. Verificación: la línea `API_URL` del panel debe mostrar la URL de Render; entonces el badge pasa a "backend OK".
- **Impacto en ADN:** Toda variable `VITE_*` (o build-time de cualquier bundler) exige redeploy para tomar efecto. El frontend expone su `API_URL` en pantalla a propósito, como chivato de configuración.

---

## 14. BITÁCORA DE CONTINUACIÓN (para retomar en otra sesión)

> Esta sección es el "save point". Si retomas el proyecto en un chat o agente nuevo, lee esto primero y tendrás el estado completo sin reconstruir nada.

### 14.1 — Qué está HECHO (✅ Fase 0, verificada)
- **Repo creado y poblado:** `github.com/berthidalgo/isht-webapp`, rama `main`, estructura monorepo completa del §3-BIS (backend/ frontend/ etl/ ml/ + README, MASTER-PLAN, .gitignore).
- **Backend desplegado y vivo:** `https://isht-webapp.onrender.com`
  - `GET /health` → `{"status":"ok","fase":"live","modelo":false}` ✅
  - `GET /docs` → Swagger UI operativo ✅
  - Endpoints de datos (`/api/cuencas`, `/api/indice`) devuelven 404 claros hasta que el ETL deposite artefactos. `/api/predecir` devuelve 503 hasta entrenar el modelo. Esto es correcto (degradación con gracia).
  - Render: plan free, Root Directory=`backend`, `PYTHON_VERSION=3.11.9`.
- **Frontend desplegado y vivo:** `https://isht-webapp.vercel.app`
  - Mapa Leaflet de Perú + panel lateral + badge de estado.
  - Badge muestra **"backend OK"** y `API_URL: https://isht-webapp.onrender.com` ✅ (conexión front-back confirmada).
  - Vercel: Root Directory=`frontend`, env `VITE_API_URL=https://isht-webapp.onrender.com` (Production).
- **DoD F0 cumplido al 100%:** dos URLs públicas, el front habla con el back.
- **Pipeline de deploy probado de extremo a extremo:** push a GitHub → Render y Vercel publican. Validado con los 3 fixes documentados (lecciones #ISHT-3, 4, 5).

### 14.2 — Qué SIGUE (◀ Fase 1, el trabajo inmediato)
El objetivo de F1 es producir los artefactos de datos: `cuencas.geojson`, `indice_isht.json`, `tabla_features.parquet` (159 filas), y copiarlos a `backend/data/`.

**Orden dentro de F1 (el primero es BLOQUEANTE de admisibilidad):**
1. **🔴 GeoPerú/GEOIDEP vía WFS — cuencas Pfafstetter (159 UH).** Sin esto = eliminación del concurso. **Decisión del usuario:** vía elegida = *"script que verifica endpoints WFS y descarga, corrido en su máquina"* (opción 1). **El script aún NO se ha entregado.** Es el siguiente entregable concreto.
   - Restricción técnica: los dominios `.gob.pe` NO son alcanzables desde el entorno del agente (firewall). La verificación de endpoints la hace el usuario en su máquina.
   - Endpoints candidatos (de §8-BIS del doc estratégico y §4 de este doc): ANA `geosnirh.ana.gob.pe/server/rest/services/Público/SERV_AguasContiLoticos/MapServer`; visor `visor.geoperu.gob.pe`; catálogo GEOIDEP. Plan B: descarga directa SHAPE-ZIP / ArcGIS REST `?f=geojson`.
   - El loader `etl/loaders/load_geoperu_wfs.py` ya está en el repo con backoff + cache; falta la URL/typename reales verificados y correr el ETL.
2. **PISCO_HyM_GR2M (oferta).** NetCDF desde HydroShare/Figshare (DOI: 10.3390/w13081048). **Estado: el usuario aún no confirma si lo tiene descargado** (pregunta quedó pendiente; se atiende después del paso 1). Loader: `etl/loaders/load_oferta.py` (stub).
3. **Demanda por cuenca** (INEI población + ANA derechos + proxy NDVI agro). Loader: `etl/loaders/load_demanda.py` (stub).
4. **Transforms:** balance (oferta−demanda→brecha→semáforo, umbrales rojo<−0.2/azul>0.1) e índice 0-100. `etl/transforms/{balance,indice}.py` (stubs).
5. **Export + validate:** escribir artefactos y validar 159 cuencas/rangos/nulos. `etl/exporters/export_artifacts.py` + `etl/validate.py` (stubs).
6. **Copiar artefactos a `backend/data/`** y verificar que `/api/cuencas` y `/api/indice` ya responden 200 en Render.

> El prompt pre-escrito de F1 está en §7 (Prompt Fase 1 — ETL con GeoPerú). El código del núcleo ML (F2) ya está completo en §5.5 y en `ml/train_model.py`. El `backend/main.py` ya tiene TODOS los endpoints de F3 implementados (§8), así que F3 es mayormente "verificar que sirve los artefactos", no escribir de cero.

### 14.3 — Decisiones tomadas en esta sesión (no re-litigar)
- **Sin base de datos (Supabase/Postgres) para el concurso.** 159 cuencas = pocos MB → archivos `.geojson`/`.json` versionados en el repo, servidos por Render. Postgres+PostGIS es backlog post-concurso (escalamiento nacional). Menos infraestructura = menos superficie de fallo el día del pitch.
- **Stack de versiones clavado:** Python 3.11.9; deps en `backend/requirements.txt` (fastapi 0.115, pydantic 2.9.2, xgboost 2.1.1, pandas 2.2.3, scikit-learn 1.5.2).
- **`modelo.pkl` y `.geojson` SÍ se versionan** en el repo (livianos). Solo los crudos pesados (NetCDF, rasters) van gitignored.

### 14.4 — Estado de tokens / handoff
- Esta bitácora se creó a pedido del usuario para poder continuar en otra sesión por límite de tokens.
- **Primera acción al retomar:** entregar el script de verificación+descarga WFS de GeoPerú (paso 14.2.1), corrible en la máquina del usuario, que pruebe los endpoints candidatos, reporte cuál responde, y descargue las cuencas a `etl/data/raw/`.
...
- [x] 2. Fase actual y DoD definidas → F0 cerrada; F1 en curso, §6 y §14
- [x] 3. Triggers de transición verificables → DoD por fase (F0 verificado en vivo)
- [x] 4. Stack zero-install y gratuito → §3.3 (Render free + Vercel, confirmado)
- [ ] 5. Dataset de evals (calibración) → §9 (pendiente F2)
- [x] 6. Decisiones arquitectónicas documentadas → notas en §3, §5, §8 + §14.3
- [x] 7. Backlog de visión → LSTM/GWR/DBSCAN (§5.3), 3 motores del plan estratégico
- [x] 8. Prompts de cada fase pre-escritos → §7
- [ ] 9. Código separación responsabilidades, funciones <20 líneas, tests → estructura dada (ETL pendiente F1)
- [x] 10. Red Team review de la fase actual → §11 + lecciones #ISHT-3,4,5
- [⚠️] 11. Anti-Ban + Heartbeat → §10 (adaptado: cache+idempotencia+UptimeRobot; UptimeRobot pendiente F5)
...
- [x] Backend (Render) + Frontend (Vercel) públicos y estables → ✅ vivos y conectados
- [ ] **Cumplimiento GeoPerú demostrable** (cuencas vía WFS) — admisibilidad → 🔴 SIGUIENTE PASO (F1)
- [ ] Modelo valida contra evals (Ica roja, La Libertad sana) → pendiente F2
- [ ] Captura/GIF + URL del tablero para campo 10 (anexo visual) → pendiente F4/F5
- [ ] 10 campos de las bases redactados, con nota de cumplimiento GeoPerú → pendiente F5
- [ ] **Propuesta registrada en facilita.gob.pe/t/52313 dentro de plazo** → pendiente F5
